...
export default class KanbanDbService implements IKanbanDbService {
    ...
    modify(kanbanItem: KanbanDbItem): GenericResponse<string> {
        try {
            ...
            const stmt = this.#db.prepare(`UPDATE ${TABLE_KANBAN_ITEMS} SET title = ?, description = ?, priority = ?, status = ?, time = ?, start = ?, duration = ?, "order" = ? WHERE id = ?`);
            stmt.run(kanbanItem.title, kanbanItem.description, kanbanItem.priority, kanbanItem.status ?? kanbanCard.data.status, kanbanItem.time,
                kanbanItem.start ?? kanbanCard.data.start, kanbanItem.duration ?? kanbanCard.data.duration, kanbanItem.order ?? kanbanCard.data.order, kanbanCard.data.id);
